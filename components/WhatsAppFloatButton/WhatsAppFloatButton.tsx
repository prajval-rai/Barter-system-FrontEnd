...
export default function WhatsAppFloatButton() {
  const phoneNumber = "918850005260"; // country code + number, no + or spaces
  ...
  useEffect(() => {
    // Show the tooltip once per day per browser, not on every page load
    ...
    if (!lastDismissed || Date.now() - Number(lastDismissed) > oneDay) {
      const timer = setTimeout(() => setShowTooltip(true), 1500); // small delay so it doesn't pop in instantly
    ...
    <>
      ...
      <style>{`
        .wa-float-wrapper {
          ...
          /* Desktop default: bottom-right, no nav bar to clear */
          bottom: 24px;
          right: 24px;
          left: auto;
        ...
        /* Mobile: bottom-left, cleared above the bottom nav bar,
           since mobile has both a bottom nav and an existing right-side FAB */
        @media (max-width: 640px) {
          ...
          .wa-float-tooltip {
            animation-name: wa-tooltip-in-mobile;
          }
          @keyframes wa-tooltip-in-mobile {
            from { opacity: 0; transform: translateX(-6px); }
            to   { opacity: 1; transform: translateX(0); }
          }
        }
